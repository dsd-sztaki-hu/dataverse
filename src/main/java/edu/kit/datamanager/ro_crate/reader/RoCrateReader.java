...
public class RoCrateReader {
    ...
    /**
     * This function will parse the provided roCrateJsonString and then
     * build the relation between the entities.
     *
     * @param roCrateJsonString the String representation of the ro-crate-medata.json to be parsed
     * @return the parsed RO-crate
     */
    public RoCrate parseCrate(String roCrateJsonString) {
        // get the content of a ro-crate-medata.json
        ObjectNode metadataJson = reader.readMetadataJson(roCrateJsonString);
        JsonNode context = metadataJson.get(PROP_CONTEXT);
        CrateMetadataContext crateContext = new RoCrateMetadataContext(context);
        RoCrate crate = new RoCrate();
        crate.setMetadataContext(crateContext);
